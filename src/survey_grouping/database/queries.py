...
from ..models.address_stats import AddressStats, DistrictStats, VillageStats
...
class AddressQueries:
    ...
    async def get_district_stats(self, district: str) -> DistrictStats | None:
        """取得區級統計摘要"""
        try:
            all_stats = await self.get_address_stats(district)

            district_stat = next((s for s in all_stats if s.level == "district"), None)
            ...
            for stat in all_stats:
                if stat.level == "village":
                    village_stats.append(VillageStats.from_address_stats(stat))
            ...
            return DistrictStats(
                ...
                total_neighborhoods=(district_stat.neighborhood_count or 0),
